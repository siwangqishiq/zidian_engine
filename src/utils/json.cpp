...
namespace zidian{
    ...
    int JsonParser::doParseObject(std::wstring &jsonStr ,int beginPostion){
        ...
        while(readIndex < jsonStr.size()){
            ...
            if(state == ParserState::INIT){//初始状态
                ...
                Log::e("util_json", "parse json error position : %d", readEndPosition);
                return -1;
            }//end if
    ...
    int JsonParser::onReadArrayStringItem(std::wstring &value ,int &position){
        if(currentJsonArray == nullptr){
            Log::w("util_json", "onReadArrayStringItem parse json error in position : %d", 
                position);
            return -1;
        }
    ...
    int JsonParser::onReadArrayNumItem(std::wstring &value ,int &position){
        if(currentJsonArray == nullptr){
            Log::w("util_json", "onReadArrayNumItem parse json error in position : %d", 
                position);
            return -1;
        }
        ...
        try{
            ...
            Log::e("util_json", "onReadArrayNumItem exception : %s", 
                e.what());
            value = L"";
            return -1;
        }
    ...
    int JsonParser::onReadStringItem(std::wstring &key , std::wstring &value , int &position){
        if(currentJsonObject == nullptr){
            Log::w("util_json", "onReadStringItem is null");
            return -1;
        }
    ...
    int JsonParser::onReadNumItem(std::wstring &key , std::wstring &value , int &position){
        if(currentJsonObject == nullptr){
            Log::w("util_json", "onReadNumItem parse json error in position %d", position);
            return -1;
        }
    ...
    int JsonParser::onReadJsonArrayItem(std::wstring &key ,
        ...
        if(currentJsonObject == nullptr){
            Log::e("util_json", "onReadJsonArrayItem parse json error in position %d", position);
            return -1;
        }
    ...
    int JsonParser::onReadJsonObjectItem(std::wstring &key , 
        ...
        if(currentJsonObject == nullptr){
            Log::e("util_json", "onReadJsonObjectItem parse json error in position %d", position);
            return -1;
        }
    ...
    int JsonParser::onReadArrayJsonObjectItem(std::shared_ptr<JsonObject> jsonObject, 
        ...
        if(currentJsonArray == nullptr){
            Log::e("util_json", "parse json array error position %d", position);
            return -1;
        }
    ...
    std::shared_ptr<JsonObject> JsonParser::parseJsonObject(std::wstring &jsonStr){
        ...
        Log::e("util_json", "parse json array error");
